...
Deno.test("filterReadingsByMealWindows - returns all if no windows defined", () => {
  ...
  assertEquals(
    filtered.length,
    2,
    "Should return all readings when no windows",
  );
});
...
Deno.test("detectStatisticalOutliers - handles small datasets", () => {
  ...
  assertEquals(
    filtered.length,
    2,
    "Should return all readings when < 3 readings",
  );
});
...
Deno.test("filterOutliers - applies detection per group", () => {
  ...
  assertEquals(
    minutes.filter((m) => m >= 475 && m <= 485).length,
    5,
    "Fasting cluster intact",
  );
  assertEquals(
    minutes.filter((m) => m >= 715 && m <= 725).length,
    3,
    "Meal cluster intact",
  );
});
...
Deno.test("validateSchedulesAgainstWindows - filters schedules outside windows", () => {
  ...
  assertEquals(
    kept.includes("2026-01-05T09:30:00.000Z"),
    false,
    "Should not include 10:30 schedule",
  );
});
...
Deno.test("validateSchedulesAgainstWindows - returns all if no windows", () => {
  ...
  assertEquals(
    filtered.length,
    1,
    "Should return all schedules when no windows",
  );
});
...
Deno.test("Integration - full filtering pipeline", () => {
  ...
  assertEquals(
    filtered.length,
    8,
    "Should remove 6:00 and 23:00 by window filter",
  );
  ...
  assertEquals(
    filtered.length,
    8,
    "No statistical outliers after window filtering",
  );
  ...
  assertExists(
    minutes.find((m) => m >= 475 && m <= 485),
    "Fasting cluster intact",
  );
  assertExists(
    minutes.find((m) => m >= 615 && m <= 625),
    "Meal 1 cluster intact",
  );
  assertExists(
    minutes.find((m) => m >= 735 && m <= 740),
    "Meal 2 cluster intact",
  );
});
...
Deno.test("adjustSchedulesToMealWindows - keeps schedules already inside windows", () => {
  ...
  assertEquals(
    adjusted[0].scheduled_at,
    "2026-01-05T07:45:00.000Z",
    "Should not modify time",
  );
});
...
Deno.test("adjustSchedulesToMealWindows - discards schedules too far from windows", () => {
  ...
  assertEquals(
    adjusted[0].minute_of_day,
    8 * 60,
    "Should keep the one inside window",
  );
});
...
Deno.test("adjustSchedulesToMealWindows - discards schedules with no matching window", () => {
  ...
  assertEquals(
    adjusted.length,
    0,
    "Should discard schedules without matching windows",
  );
});
...
Deno.test("fillMissingMealWindowReminders - adds default for uncovered windows", () => {
  ...
  const userTypes = new Map([[
    "test-user",
    new Set(["fasting", "1hr_after_meal"]),
  ]]);
  ...
  assertEquals(
    gapFillers.length,
    1,
    "Should create one gap filler for uncovered window",
  );
  assertEquals(
    gapFillers[0].measurement_type,
    "1hr_after_meal",
    "Should fill meal window",
  );
  assertEquals(
    gapFillers[0].is_default_reminder,
    true,
  ...
  assertEquals(
    ...
    12,
    "Should be 20 min before end - hour",
  );
  assertEquals(
    reminderDate.getUTCMinutes(),
    40,
    "Should be 20 min before end - minutes",
  );
});
...
Deno.test("fillMissingMealWindowReminders - does not duplicate covered windows", () => {
  ...
  assertEquals(
    gapFillers.length,
    0,
    "Should not create filler for already covered window",
  );
});

...
const fixturePath = new URL(
  "../_fixtures/glucose-measurments.json",
  import.meta.url,
);
...
Deno.test("Fixture validation - filter real glucose measurements", () => {
  ...
  // Get unique user IDs
  const userIds = [
    ...new Set(fixtureData.map((r: any) => r.user_id)),
  ] as string[];
  ...
  for (const userId of userIds) {
    ...
    console.log(
      `   ✓ Outlier filtering: removed ${removedByOutliers} readings`,
    );
    console.log(`   ✓ Final: ${outlierFiltered.length} valid readings`);

    // Assertions
    assert(
      outlierFiltered.length > 0,
      "Should have some valid readings after filtering",
    );
    assert(
      outlierFiltered.length <= readings.length,
    ...
    for (const reading of outlierFiltered) {
      const applicableWindows = windows.filter(
        (w) =>
          w.day_of_week === reading.day_of_week &&
          w.measurement_type === reading.measurement_type,
      );
      ...
      const inWindow = applicableWindows.some((w) => {
        ...
        return reading.minute_of_day >= startMinute &&
          reading.minute_of_day <= endMinute;
      });

      assert(
        inWindow,
        `Reading at ${reading.minute_of_day} min should be within a window`,
      );
    }
  }
});
...
Deno.test("Fixture validation - measurement type distribution", () => {
  ...
  const afterMeal = readings.filter((r) =>
    r.measurement_type === "1hr_after_meal"
  );

  console.log(
    `   Original: ${fasting.length} fasting, ${afterMeal.length} after meal`,
  );
  ...
  const fastingFiltered = filtered.filter((r) =>
    r.measurement_type === "fasting"
  );
  const afterMealFiltered = filtered.filter((r) =>
    r.measurement_type === "1hr_after_meal"
  );

  console.log(
    `   Filtered: ${fastingFiltered.length} fasting, ${afterMealFiltered.length} after meal`,
  );
...
Deno.test("Fixture validation - day of week coverage", () => {
  ...
  assert(
    byDay.size >= 3,
    "Should have readings from at least 3 different days",
  );
});
